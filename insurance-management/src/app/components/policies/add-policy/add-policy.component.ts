...
export class AddPolicyComponent implements OnInit, AfterViewInit {
  ...
  addPolicy() {
    ...
    if (this.policyForm?.invalid) {
      const email = this.policyForm.get('beneficiaries')?.value.map((beneficiary: any) => beneficiary.email);
      console.log('email ' + this.validateEmail(email));
      if (email && email.length > 0 && !this.validateEmail(email)) {
        this.beneficiariesMessage = 'Email is not valid';
     ...
     if (this.beneficiaries.length > 0) {
      ...
      if (totalShare < 100) {
        this.global.showError('Total share percentage ' + totalShare + ' must be 100%.', 'Close');
        this.beneficiariesMessage = 'Total share percentage ' + totalShare + ' must be 100%.'
        return;
      }
    }

    this.policyHolderServices.createPolicy(this.policyForm.value)
      .pipe(first())
      .subscribe(
        {
          next: (data: any) => {
            this.global.showSuccess('Policy added!', 'Policy successfully added!');
            this.router.navigate(['/policies']);
          },
          error: (error) => {
            this.global.showError('Failed to add a policy', 'Failed to add a policy');
  ...
  validPolicyDetails(): boolean {
    ...
    if (policyNumber == '' || insuranceCompany == '' || policyType == '') {
      ...
      this.global.showError('Enter Policy details first', 'Close');
      return false;
    }
    return true;
  }
